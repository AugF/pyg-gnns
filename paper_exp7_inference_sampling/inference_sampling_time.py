...
dir_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "inference_sampling_time")
...
for i, mode in enumerate(['cluster', 'graphsage']):
    ...
    for data in small_datasets:
        for alg in algs:
            if mode == 'cluster':
                cmd = "python /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_inference_sampling.py --device cuda:{} --mode {} --model {} --data {} --epochs 20 --batch_partitions {} >{} 2>&1"
                ...
                cmd = "python /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_inference_sampling.py --device cuda:{} --mode {} --model {} --data {} --epochs 20 --batch_size {} >{} 2>&1"
    ...
    with open("inference_sampling_" + mode + "_time.sh", "w") as f:
