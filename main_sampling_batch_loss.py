...
from utils_sampling import pics_losses, sampling_modes, algorithms
...
def test():  # Inference should be performed on the full graph.
    ...
    correct = y_pred[data.test_mask].eq(y_true[data.test_mask]).sum().item()
        ...
        with torch.autograd.profiler.emit_nvtx(record_shapes=not args.no_record_shapes):
            ...
            for epoch in range(args.epochs):
                for batch in train_loader:
                    ...
                    print("Batch", cnt)
                    # print(f"Batch {cnt}, train_loss: {train_loss}, eval_loss: {eval_loss}")
            ...
            file_prefix = "loss_file/" + args.model + '_' + args.dataset + '_' + args.mode
            data = np.array([train_losses, eval_losses])
            np.save(file_prefix + '.npy', data)
            pics_losses(data, "Batch", file_prefix + ".png", algorithms[args.model] + " " + sampling_modes[args.mode])
