...
It is noteworthy that some complex GNNs like GAT, GaAN and ChebNet require more than one message passing phase in each GNN layer.
%
We regard every message passing phase in a GNN layer as a \emph{sub-layer}.
%
Take GAT \cite{huang2018_gat} as an example.
%
A GAT layer has two sub-layers and each sub-layer has $K$ heads under the multi-head attention mechanism.
%
For the GAT layer $l$, it first transforms every hidden vector received from the previous GAT layer $\boldsymbol{h}^l_x$ with a weight matrix $\textcolor{blue}{\boldsymbol{W}^{l}_{(k)}}$ for each head $k$ ($\parallel$ represents the concatenation operator):
%
\begin{equation}
   \hvec{x} = \parallel_{k=1}^K \Param{\MyMat{W}^l_{(k)}}\MyVec{h}^l_x, \quad \text{denoting } \hvec{x}[k]=\Param{\MyMat{W}^l_{(k)}}\MyVec{h}^l_x.
\end{equation}
%
The GAT layer $l$ then calculates the hidden vector $\MyVec{h}^{l,0}_x$ of every vertex $v_x$ in the first sub-layer as:
%
...
\end{equation}
%
$\MyVec{h}^{l,0}_x$ stores the sum of adjacent edge weights of all heads for $v_x$.
%
It is passed to the second sub-layer as ($\delta$ represents the activation function):
%
\begin{equation}
    \begin{aligned}
        ...
        \MyVec{h}^{l,1}_x &= \gamma^{l,1}(\MyVec{h}^{l,0}, \MyVec{s}^{l,1}_x) = \delta(\MyVec{s}^{l,1}_x).
    \end{aligned}
\end{equation}
%
In the GAT layer $l$, the graph neuron corresponding to $v_x$ first transforms the hidden vector of $v_x$ from the previous layer $\boldsymbol{h}^l_x$ with a weight matrix $\textcolor{blue}{\boldsymbol{W}^{l,k}}\boldsymbol{h}_x^l$.
calculates the weight (i.e., attention) of every adjacent edge dynamically with 
        ...
        \begin{tabular}{clr}
            ...
            \begin{scriptsize}
                $\begin{aligned}[t]
                         ...
                         & \text{Sub-layer 1:}                                                                            \\
                         & \boldsymbol{h}_x^{l+1} = \delta(\boldsymbol{s}_x^{l,1})
                    \end{aligned}
                $
            \end{scriptsize}                                                                                                                                                                                   &
            $
            ...
            \begin{scriptsize}
                $\begin{aligned}[t]
                         ...
                         & \boldsymbol{h}_x^{l+1} = \textcolor{blue}{\boldsymbol{W}^l_o} [\boldsymbol{h}_x^l \parallel (\boldsymbol{g}_{x}^l \odot \boldsymbol{h}_{x}^{l, 1})] + \textcolor{blue}{\boldsymbol{b}^l_o}
                    \end{aligned}$
            \end{scriptsize}                                                                                                                                                                                   &
