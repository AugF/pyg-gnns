...
%
To find out how the batch size affected the accuracy, we trained GNNs under different batch sizes and compared their accuracy with the full-batch training in \figurename~\ref{fig:exp_sampling_relative_batch_size_accuracy}.
%
The experiemental results on other datasets were similar.
%
We used the same hyper-parameters for the sample-based training and the full-batch training.
%
For each dataset and each GNN, we chose the hyper-parameters that achieved the highest accuracy for the full-batch training, according to the experimental results in Section~\ref{sec:effects_of_hyper-parameters_on_performance}.

\begin{figure}[H]
    \centering
    \subfloat[Neighbor sampler on \texttt{amc}]{\includegraphics[height=5cm]{figs/experiments/exp_graphsage_sampling_accuracy_on_amc.pdf}}
    \subfloat[Neighbor sampler on \texttt{fli}]{\includegraphics[height=5cm]{figs/experiments/exp_graphsage_sampling_accuracy_on_fli.pdf}} \\
    \subfloat[Cluster sampler on \texttt{amc}]{\includegraphics[height=5cm]{figs/experiments/exp_cluster_sampling_accuracy_on_amc.pdf}}
    \subfloat[Cluster sampler on \texttt{fli}]{\includegraphics[height=5cm]{figs/experiments/exp_cluster_sampling_accuracy_on_fli.pdf}}
...
%
But there was also a little exceptions (cluster sampler on \texttt{amc}, relative batch size was 50\%).
%
...
% added by wangyunpan, 2020-12-23, ----end----
