...
According to \tablename~\ref{tab:gnn_overview_edge} and \tablename~\ref{tab:gnn_overview_vertex}, the time complexity of the messaging function $\phi^l$ and the updating function $\gamma^l$ were linear to each hyper-parameter separately.
%
If we increased one of the hyper-parameters and fix the others, the training/inference time should increase linearly in theory.
...
\begin{figure}[htb]
    ...
    \caption{Distribution of the wall-clock training time of 50 epochs on different datasets. GaAN crashed due to out of memory exception on the \texttt{cph} dataset.}
...
To evaluate the effects of hyper-parameters on the accuracy, we measured the accuracy of the typical GNNs with varying hyper-parameters.
%
...
%
When the time complexity of $\phi^l$ was high, the messaging step was the performance bottleneck.
%
...
%
Taking GCN as an example, \figurename~\ref{fig:inference_analysis_on_layer_edge_level_of_gcn} shows the results of GCN inference on the \emph{layer} level and the \emph{step} level.
%
By cross-comparing \figurename~\ref{fig:exp_vertex_edge_cal_proportion}(a), 
...
\begin{figure}[H]
    \centering
    \subfloat[Layer Level]{\includegraphics[width=0.3\columnwidth]{figs/experiments/exp_inference_full_avg_degree_on_vertex_edge_cal_time_gcn.pdf}}
    \subfloat[Effects of the average degree]{\includegraphics[width=0.3\columnwidth]{figs/experiments/exp_inference_full_avg_degree_on_vertex_edge_cal_time_gcn.pdf}}
    \subfloat[Edge Level]{\includegraphics[width=0.3\columnwidth]{figs/experiments/exp_inference_full_edge_calc_decomposition_gcn.pdf}}
    \caption{Time breakdowns on the layer level and edge level, and effects of average degree on the edge/vertex calculation of GCN inference.}
    \label{fig:inference_analysis_on_layer_edge_level_of_gcn}
\end{figure}
...
\begin{figure}[H]
    ...
    \subfloat[GAT]{\includegraphics[height=4cm]{figs/experiments/exp_inference_full_top_basic_ops_gat.pdf}}
...
\paragraph{Memory Usage of Inference}
...
%
How to automatically select a proper batch size was a topic worth further studying.
