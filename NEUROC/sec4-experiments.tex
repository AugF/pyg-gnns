...
Since the real-world graphs have more edges than edges ($|\mathcal{E}| > |\mathcal{V}|$), the time complexity of the edge calculation stage affects more than the vertex calculation stage.
...
$dim(\MyVec{h^0})$ and $dim(\MyVec{h}^1)$ are solely determined by the dataset with $d^0_{in} = dim(\MyVec{h}^0_x) = dim(\MyVec{v_x})$ and $d^1_{out} = dim(\boldsymbol{h}^2_x)=\#Class$.
%
For GCN and GGNN, the only modifiable hyper-parameter is the dimension of the hidden vectors $dim(\boldsymbol{h}^1_x)$ that are both the output hidden vectors for the GNN layer 0 and the input hidden vector for the GNN layer 1.
%
According to the time complexity analysis, if we fix other hyper-parameters but only increase $dim(\boldsymbol{h}^1_x)$, the computational costs of the GNN layer 0 and the GNN layer 1 both increase linearly with $dim(\boldsymbol{h}^1_x)$, causing the training time of the whole GNN also increasing linearly. 
...
The dimension of the hidden vectors $dim(\MyVec{h}^1_x)$ is determined as $dim(\boldsymbol{h}^1_x) = K d_{head}$.
...
Its time complexity is affected by $dim(\boldsymbol{h}^1_x)$ ($d^0_{out} = d^1_{in} = dim(\boldsymbol{h}^1_x)$), $d_a$, $d_v$, $d_m$, and the number of heads $K$.
\figurename~\ref{fig:exp_hyperparameter_on_vertex_edge_phase_time_gaan} demonstrates that the training time increases linearly with the hyper-parameters, except for $dim(\boldsymbol{h}^1_x)$.
As $dim(\boldsymbol{h}^1_x)$ increases, the training time increases first slightly and then linearly.
We observe similar phenomena in GCN, GGNN, and GAT with low hyper-parameter values.
When the values of hyper-parameters are too low, the GNN training cannot make full use of the computing power of the GPU.
When the values of hyper-parameters become high enough, the training time increases linearly, supporting the time complexity analysis.
...
The memory usage also increases linearly as the hyper-parameters increase for all GNNs, except for GaAN on $dim(\boldsymbol{h}^1_x)$.
As the hidden vectors $\boldsymbol{h}^1_x$ consume a small proportion of memory in GaAN, the growth in the memory usage is not noticeable until $dim(\boldsymbol{h}^1_x)$ is large enough.
...
$d^0_{in}=dim(\boldsymbol{v}_x)$, $d^0_{out}=d^1_{in}=64$ and $d^1_{out}=\#Class$ and $dim(\boldsymbol{v}_x) \gg \#Class$.
