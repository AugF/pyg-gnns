...
\begin{reply}
    ...
    %
    The efficiency of inference (including inference time and memory usage) is also important for deep learning researchers and engineers.
    %
    To discuss the efficiency of GNN inference on GPUs, we have added a new subsection (Section 2.5 ``Inference with GNNs'') in the revised manuscript to briefly review how to perform inference with GNNs.
    %
    We have also added corresponding analysis on GNN inference in every subsection of Section 4 ``Evaluation Results and Analysis'' in the revised manuscript:
    %
    \begin{enumerate}
        \item In Section 4.1 ``Effects of Hyper-parameters on Performance'', 
    \end{enumerate}
