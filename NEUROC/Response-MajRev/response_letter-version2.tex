...
\begin{reply}
	Thank you for pointing out missing references. Computer vision is indeed an important application area of graph neural networks. We have added the mentioned references in Section 1 ``Introduction'' in the revised manuscript.
\end{reply}
...
\begin{reply}
    ...
    %
    There is no clear winner in terms of accuracy.
    %
    GaAN that has the highest complexity performs slightly better than the other GNNs.
    %
    Simple GNN models (such as GCN) can still achieve good acuuracy with proper hyper-parameter settings.
...
\begin{reply}
    ...
    %
    Results of the other datasets are available in Section 4.4.4 ``Effects on Accuracy'' in the revised manuscript.
    %
...
\begin{reply}
    ...
    %
    As suggested, we additionally evaluate the effects of model complexity on accuracy in Section 4.1 ``Effects of Hyper-parameters on Performance'' in the revised manuscript.
    %
    We add experimental results on how the hyper-parameters of GNNs affect the accuracy.
    %
    ...
    %
    We presented the experimental results in a new subsection Section 4.4.4 ``Effects on Accuracy''.
\end{reply}
