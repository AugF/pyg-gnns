        ...
        with torch.autograd.profiler.emit_nvtx(record_shapes=not args.no_record_shapes):
            ...
            for epoch in range(args.epochs):
                for batch in train_loader:
                    ...
                    if cnt % 10 == 0: # cnt必须是10的倍数    
                        train_acc, val_acc, test_acc = test()
                        print(f'Batch: {cnt:02d}, Loss: {ls:.4f}, Train: {train_acc:.4f}, '
                                f'Val: {val_acc:.4f}, test: {test_acc:.4f}')
                        losses.append(ls)
                ...
                if cnt >= 300:
                    break
            np.save("sampling_valids/" + args.model + "_" + args.dataset + "_" + args.mode + "_batch.npy", np.array([losses, train_acces, test_acces]))
