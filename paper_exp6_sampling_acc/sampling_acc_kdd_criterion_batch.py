...
dir_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "kdd_criterion_batch")
...
for i, mode in enumerate(['cluster', 'graphsage']):
    ...
    for alg in algs:
        for data in small_datasets:
            ...
            if mode == 'cluster':
                cmd = "python -u /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_sampling_kdd_criterion_batch.py --mode {} --model {} --data {} --device cuda:{} --batch_partitions {} {} >>{} 2>&1"
                ...
                cmd = "python -u /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_sampling_kdd_criterion_batch.py --mode {} --model {} --data {} --device cuda:{} --batch_size {} {} >>{} 2>&1"
    ...
    with open("sh_" + mode + "_kdd_criterion_batch.sh", "w") as f:
