...
dir_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "batch_acc")
...
for i, mode in enumerate(['cluster', 'graphsage']):
    ...
    for alg in algs:
        for data in small_datasets:
            ...
            if mode == 'cluster':
                cmd = "python -u /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_sampling_batch_acc.py --runs 1 --epochs 500 --mode {} --model {} --data {} --device cuda:{} --batch_partitions {} {} >>{} 2>&1"
                ...
                full_sh = f"python -u /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main.py --runs 1 --epochs 1000 --mode {mode} --model {alg} --data {data} --device cuda:{i} {config_str} >>{full_log} 2>&1"
                sh_commands.append(full_sh)
            else:
                cmd = "python -u /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_sampling_batch_acc.py --runs 1 --epochs 500 --mode {} --model {} --data {} --device cuda:{} --batch_size {} {} >>{} 2>&1"
    ...
    with open("batch_acc_" + mode + ".sh", "w") as f: # w每次打开一个新文件
