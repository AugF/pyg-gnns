...
config_paras = pd.read_csv("/home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/paper_exp1_super_parameters/early_stopping2/acc_res/max_acc.csv", index_col=0)
...
dir_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "batch_acc_cum_fix_batch")
...
for i, mode in enumerate(['cluster', 'graphsage']):
    ...
    for alg in algs:
        for data in small_datasets:
            ...
            if mode == 'cluster':
                cmd = "python -u /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_sampling_batch_acc_cum_fix_batch.py --epochs 10000 --mode {} --model {} --data {} --device cuda:{} --batch_partitions {} {} >>{} 2>&1"
                ...
                if os.path.exists(full_log):
                    continue
                full_sh = f"python -u /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_full_batch_acc_cum.py --runs 1 --epochs 1000 --model {alg} --data {data} --device cuda:{i} {config_str} >>{full_log} 2>&1"
                sh_commands.append(full_sh)
            else:
                cmd = "python -u /home/wangzhaokang/wangyunpan/gnns-project/pyg-gnns/main_sampling_batch_acc_cum_fix_batch.py --epochs 10000 --mode {} --model {} --data {} --device cuda:{} --batch_size {} {} >>{} 2>&1"
    ...
    with open("batch_acc_cum_" + mode + "_fix_batch.sh", "w") as f:
