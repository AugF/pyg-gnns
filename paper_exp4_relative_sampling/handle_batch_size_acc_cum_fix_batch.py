...
dir_in = "batch_acc_cum_fix_batch"
dir_out = "acc_cum_res_fix_batch"
...
for data in datasets:
    for alg in algs:
        for mode in modes:
            ...
            # 获取full
            file_path = file_path = os.path.join(dir_in, '_'.join([mode, alg, data]) + "_full.log")
            # 读取日志文件，获取bs_times, bs_accs: dims=100
            with open(file_path) as f:
            ...
            for k in df_times.keys():
                print(k, len(df_times[k]))
            # plot
            # 1. 画关于时间的图像
            df = pd.DataFrame(df_times, index=range(100))
            # df.to_csv(dir_out + "/"  + data + "/" + alg + "_" + mode + "_times.csv")
            fig, ax = plt.subplots()
            ax.set_ylabel('Times(s)')
            ax.set_xlabel("Number of Batches")
            # markers = 'oD^sdp'
            for j, c in enumerate(df.columns):
                df[c].plot(ax=ax, label=c, rot=0)
            ax.legend()
            fig.tight_layout() # 防止重叠
            fig.savefig(dir_out + "/"  + data + "/" + alg + "_" + mode + "_times.png")
            plt.close()     
            
            # 2. 画关于精度的图像
            df = pd.DataFrame(df_accs, index=range(100))
            # df.to_csv(dir_out + "/"  + data + "/" + alg + "_" + mode + "_accs.csv")
            fig, ax = plt.subplots()
            ...
            ax.legend()
            fig.tight_layout() # 防止重叠
            fig.savefig(dir_out + "/"  + data + "/" + alg + "_" + mode + "_accs.png")
            plt.close()     
