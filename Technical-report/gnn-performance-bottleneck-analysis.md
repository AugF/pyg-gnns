...
边计算阶段可以进一步分解为collect, message, aggregate和update四个步骤, 如[@fig:step\boldsymbol{s}_in_edge_calculation]所示.
...
在步骤没有计算,只涉及数据访问.
message步骤调用用户给出的函数$\phi$完成边计算过程, 并得到每条边的消息向量$m_{j, i}^l ()$.
aggregate步骤根据每条边的目标顶点, 将目标顶点相同的消息向量通过聚合算子$\Sigma$聚合在一起, 得到每个顶点聚合向量$a_i^l (1 \leq i \leq N)$.
最后的update步骤是可选的, 其可以对聚合后的向量

![fig:step\boldsymbol{s}_in_edge_calculation](figs/illustration/step\boldsymbol{s}_in_edge_calculation.png)

**图: 边计算的步骤分解** [#fig:step\boldsymbol{s}_in_edge_calculation]
