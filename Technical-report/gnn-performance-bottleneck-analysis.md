...
**热点算子分析**: 点/边计算的各种函数$\phi, \Sigma,\gamma$由一系列基本算子构成, 基本算子被映射到GPU上的具体基本算子(例如矩阵乘法mm, 按元素数乘elementwise_kernel和张量按index选择index_select). 图[@fig:top_basic_ops]展示了各GNN中耗时比例最高的5个算子, 图例中算子顺序由平均耗时占比确定. 各算法的高耗时算子分析如下:

- GCN中矩阵乘法算子mm主要用于点计算中$\gamma$,  该算子在cph数据集上尤其耗时, 因为cph输入的顶点特征向量维度非常高, 使得Layer0的点计算中矩阵乘法的计算量很高. mul是用于边计算函数$\phi$中的数乘.  scatter_add_和gather均用于实现边计算中的聚合步骤$\Sigma$, 其中前者用于forward计算后者用于backward计算. index_select算子用于边计算中的collect步骤. 对于GCN算法来说, 用于实现边计算的算子占据了主要的耗时, 各算子之间耗时比较平均, 没有特别突出的性能瓶颈.
- GGNN中最耗时的也是矩阵乘法mm算子, 主要用于点计算$\gamma$中对于输入输出向量的线性变换和GRU中的计算. scatter_add_, index_select和gather算子也用于边计算. 而thnn_fused_gru_cell用于GRU的backward计算中. GGNN因为点计算复杂度的提升, mm算子的耗时时间明显提高.
- GAT中最耗时的5个算子均与边计算相关. inpux_put_impl_, mul和mm用于实现边计算函数$\phi$. index_select和index算子用于边计算中的collect阶段.
- GaAN中最耗时的矩阵乘法算子mm同时用于边计算和点计算, 其中边计算占主导. mul和cat用于实现边计算中的$\phi$函数.

从共性来说, *GNN计算的主要耗时还是在矩阵乘法mm, 按元素数乘mul等算子*, 因此非常适合用GPU进行计算.  边计算中的aggregate步骤虽然计算较为简单, 但因为涉及数据同步和非规整计算(不同顶点的度数差距很大), 其计算仍然耗时. 边计算中的collect步骤虽然没有任何的计算, 但是在所有的GNN中其计算耗时依然占据10%左右. aggregate步骤和collect步骤是GNN训练的计算性能瓶颈之一, 优化相应的算子将能提升所有GNN的训练效率.
...
- 根据边计算函数$\phi$的计算复杂度不同, GNN的在边计算中的性能瓶颈不同:
  - 如果$\phi$的计算复杂度较高, 性能瓶颈集中在实现$\phi$所用的基本算子. 优化相应基本算子的实现将能提升这类GNN的训练性能.
