...
图神经网络由于多个任务领域下取得高精度的结果，成为人工智能领域内的研究热点。与此同时，也陆续出现了各种各色各样的面向图深度学习的系统，这些系统在实现时都使用了各自的技巧，但是却缺少工作对图神经网络具体的性能瓶颈进行探讨。本文以图神经网络两个基本操作：边操作和点操作，从时间复杂度的角度，选取典型算法在GPU环境下通过超参数影响、训练耗时分解，内存分析和采样背景下对性能瓶颈进行了全面透彻的分析，最终，我们提出了一些对GNN系统设计者在GNN训练优化方面的建议
