...
图神经网络由于多个任务领域下取得高精度的结果，成为人工智能领域内的研究热点。在工业界，也陆续出现了各种各色各样的面向图深度学习的系统，这些系统在实现时都使用了各自的技巧，但是却缺少工作对图神经网络具体的性能瓶颈进行探讨。本文以图神经网络两个基本操作：边操作和点操作，从时间复杂度的角度，选取典型算法在GPU环境下通过超参数影响、训练耗时分解，内存分析和采样背景下对性能瓶颈进行了全面透彻的分析，最终，我们提出了一些对GNN系统设计者在GNN训练优化方面的建议

关键词：Grpah neural network, performance bottleneck analysis, deep learning, characterizaion
...
在这些图神经网络计算系统实现中，各自用了不同的性能优化技巧，然而这些性能优化技巧是否真正反映了GNN训练过程中的性能瓶颈研究还存有疑问。目前来说，对于图神经网络训练的具体性能瓶颈的分析工作很少，工作[7]分析了GCN类的算法在inference阶段的特性，同时与经典的图分析算法(PageRank)和基于MLP的经典神经网络的特性进行了对比分析，发现实际图中的顶点度数分布符合幂等律分布的特性，因此缓存高度数的顶点，有可能可以提升硬件的cache的命中率，因为向量化原子访问可以提升aggregation阶段的效率，但是该工作只选取了某个特定的GNN算法，不能很好地表示大部分的GNN的训练分析。最近的工作[6]Architectural Implications of GNNs, 基于SAGA-NN编程模型和DGL计算系统进行实验，作者认为GNN没有固定的性能瓶颈，性能瓶颈会随着数据集和算法的不同而变化，但是该工作中选择的图的阶数都是很低的情况，而且选择算法的角度也和本文大不相同。所以，GNN训练的性能瓶颈值得我们进行更多的研究

由于图神经网络每层最本质的操作实际上可以概括为aggregate和update两个操作，aggregate操作即收集邻居顶点的信息, 反映在图上即为边操作；update操作进行顶点信息的变换，反映在图上即为点操作。所以，在本文研究GNN训练的性能瓶颈时，点操作和边操作就作为了选取典型算法研究GNN算法训练的性能瓶颈的依据。在[], [], []survey中我们对大多数GNN算法进行了点边复杂度分析，将不同算法划归到不同的象限。从四个象限中选择了四个典型算法GCN, GGNN, GAT和GaAN。实验中，我们选取了六个不同特性的数据集，并考虑R-MAT生成器生成随机图，固定了四个算法统一的层数和共性超参。在探究GNN训练的瓶颈中，我们转化为讨论以下四个问题：
1. 超参数对训练用时和内存使用的影响如何？是否符合时间复杂度分析？
2. GNN训练的哪个阶段最耗时？
3. 哪个因素对GNN训练中的内存影响最大？
4. 采样技术是否可以解决GNN训练的性能瓶颈？
针对上述四个问题，我们的Key Findings为：
对于问题一，对每一个算法，我们分析了它的各个参数对训练耗时和内存使用方面的影响，实验证明，超参数对训练用时和内存使用符合时间复杂度分析
对于问题二，我们可以将训练时间划分到不同的levels: layer level, vertex/edge computation leval和basic operator leval, 我们bareakdowon的training time到不同的阶段。我们发现，GNN计算特性是符合GPU运算的，GNN训练瓶颈受平均度数的影响，现实世界下平均度数在10度以上，集中在边计算部分。边计算复杂度高时，性能瓶颈集中在边计算的基本算子；边计算复杂度低，优化collect和aggreagte可以得到性能提升
对于问题三，我们测量了peak memory usage和expansion ratio在input features dims, 图的规模下，点数和边下的影响，GNN在训练过程中的峰值内存使用可以达到数据规模的数十倍甚至数百倍，固定顶点下，GNN的峰值内存随边的增长线性增加；在网络结构各项超参数固定的情况下，更高维的输入特征向量可以降低GPU内存膨胀比例
对于问题四，为了验证采样算法的有效性，我们测量了训练时间和内存随batch_size的变化情况，我们发现，采样技术确实能够大幅度缓解内存问题，从而将GNN算法扩展到大规模图上。但是，采样技术在batch增加后，其本身会造成更多的额外时间开销，反而更加耗时
通过以上的分析，我们为GNN的系统提供以下建议：
1. 超参数在内存和训练时间上和时间复杂度上的表现一致，我们可以从理论上对性能瓶颈进行分析
2. GPU内存膨胀是制约GNN算法扩展性的最显著因素。处理大规模图时，如何有效节约内存是始终需要放在首位的事；内存和边的复杂度成线性关系
3. 对于现实世界的图，边计算是耗时瓶颈所在，边计算的collect, message, aggregate每个阶段都值得优化，尤其是message阶段，当边计算复杂度高时，是最显著的耗时；
...
1. 超参数在内存和训练时间上和时间复杂度上的表现一致，我们可以从理论上对性能瓶颈进行分析
2. 对于现实的图，边计算是耗时瓶颈所在，边计算的collect, message, aggregate每个阶段都值得优化，尤其是message阶段，当边计算复杂度高时，是最显著的耗时；
3. GPU内存膨胀是制约GNN算法扩展性的最显著因素。处理大规模图时，如何有效节约内存是始终需要放在首位的事；内存和边的复杂度成线性关系
4. 采样技术的优势在于大幅度降低峰值内存使用的开销，使大规模图神经网络训练成为可能，优化采样技术的耗时也对整个训练会带来很大的增益
