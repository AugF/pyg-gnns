...
\begin{figure}
    ...
    \caption{Training time breakdown on the layer level, including the time spent on the forward, backward and evaluation phase. Each layer is further decomposed into the vertex and the edge calculation stages.}
...
Therefore, \emph{to reduce the GNN training time, one should focus on improving the efficiency of the edge calculation}.
...
The aggregate step aggregates the message vectors with the same target vertex into a single aggregated vector $\boldsymbol{s}^l_i$ with the aggregation operation $\Sigma$.
The update step \emph{optinally} performs additional transformation on the aggregated vectors (for example, adding bias to GCN).
The aggregated vectors $\emph{s}^l_i$ (after the update step) will be fed into the vertex update function $\gamma$ as an input parameter.
