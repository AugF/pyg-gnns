...
For GAT, a GAT layer consists of two sub-layers:
the first sub-layer calculates the summation $\sum_{k \in \mathcal{N}(i)}\exp(LeakyReLU(\textcolor{blue}{\boldsymbol{a}^T} [\hat{\boldsymbol{h}}^{l,k}_i \parallel \hat{\boldsymbol{h}}^{l,k}_j] ))$, and the second sub-layer calculates $\alpha_{j, i}^k$ and $\boldsymbol{m}^l_{j,i}$.
The vertex calculation of a GAT layer is also decomposed into two parts: $\hat{\boldsymbol{h}}^{l,k}_{i}$/$\boldsymbol{h}^{l+1}_i$ is calculated for every vertex before/after the message-passing, respectively.
The edge calculation of GAT uses $\hat{\boldsymbol{h}}^{l,k}_{i}$ during the message-passing.
		...
		\begin{tabular}{ccp{8em}p{22em}r}
			...
			\begin{scriptsize}
				$\begin{aligned}[t]
                         ...
                         & \text{Sub-layer 1:}\\
						 & \alpha_{j, i, (k)} = \frac {\exp(LeakyReLU(\textcolor{blue}{\boldsymbol{a}^T} [\hat{\boldsymbol{h}}^{l}_{i,(k)} \parallel \hat{\boldsymbol{h}}^{l}_{j,(k)}] ))} {\hat{\boldsymbol{a}}^{l,0}_{i,(k)}} \\
						 & \text{Multi-head concatenation}: \boldsymbol{m}_{j, i}^{l,1} = \parallel_{k=1}^K \delta(\alpha_{j, i, (k)} \hat{\boldsymbol{h}}^{l}_{j,(k)})                                                                                                                                             \\
						 & \text{Multi-head average}: \boldsymbol{m}_{j, i}^{l,1} = \frac{1}{K} \sum_{k=1}^K \delta(\alpha_{j, i, (k)} \hat{\boldsymbol{h}}^{l}_{j,(k)})
					\end{aligned}$
			\end{scriptsize}
             &
             $
...
Each GAT layer consists of two sub-layers.
The first sub-layer calculates the attention weights of every edge $(v_i, v_j)$ $\exp(LeakyReLU(\textcolor{blue}{\boldsymbol{a}^T} [\hat{\boldsymbol{h}}^{l,k}_i \parallel \hat{\boldsymbol{h}}^{l,k}_j]$ and aggregates the attention weights of all adjacency edges of every vertex $v_i$ as $\sum_{k \in \mathcal{N}(i)}\exp(LeakyReLU(\textcolor{blue}{\boldsymbol{a}^T} [\hat{\boldsymbol{h}}^{l,k}_i \parallel \hat{\boldsymbol{h}}^{l,k}_j] ))$.
The second sub-layer uses 
...
\begin{figure}
	\centering
	\subfloat[Neighbor sampling\label{fig:gnn_sampling_neigbhor_sampling}]{\includegraphics[height=4cm]{figs/illustration/layer_sampling.pdf}}
	%
