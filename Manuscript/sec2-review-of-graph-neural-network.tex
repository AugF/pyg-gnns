...
The vertex calculation of a GAT layer is decomposed into two parts: $\hat{\boldsymbol{h}}^{l,k}_{i}$/$\boldsymbol{h}^{l+1}_i$ is calculated for every vertex before/after the message-passing, respectively.
		...
		\begin{tabular}{ccp{8em}p{22em}r}
			...
			\begin{scriptsize}
				$\begin{aligned}[t]
						 ...
						 & \text{Multi-head concatenation}: \boldsymbol{m}_{j, i}^l = \parallel_{k=1}^K \delta(\alpha_{j, i}^k \hat{\boldsymbol{h}}^{l,k}_j)                                                                                                                                             \\
						 & \text{Multi-head average}: \boldsymbol{m}_{j, i}^l = \frac{1}{K} \sum_{k=1}^K \delta(\alpha_{j, i}^k \hat{\boldsymbol{h}}^{l,k}_j)
					\end{aligned}$
			\end{scriptsize}                                                                                             &
			\makecell[r]{concat: $O(d_{in}*d_{out})$ \\
                 average: $O(K * d_{in} * d_{out})$  \\ Two sub-layers} 
             \\
			\textbf{GaAN}                                                                                                             &
			Attention                                                                                                                 &
			sum,max,mean                                                                                                              &
...
\begin{table}
	\begin{footnotesize}
		\begin{tabular}{cp{20em}r}
			...
			$O(d_{in} * d_{out})$                                                                                                                                                                                              \\
			\textbf{GAT}                                                                                                                                                                                                     &
			\makecell[l]{$\hat{\boldsymbol{h}}^{l,k}_{i} = \textcolor{blue}{\boldsymbol{W}^{l,k}} \boldsymbol{h}_i^{l}$ (pre edge calculation),\\ $\boldsymbol{h}_i^{l+1} = \boldsymbol{s}_i^l$}                                                         &
			\makecell[r]{concat: $O(d_{in}*d_{out}/K)$\\ average: $O(d_{in}*d_{out})$
             \\ Two sub-layers}
			\\
			\textbf{GaAN}                                                                                                                                                                                                    &
			\begin{scriptsize}
