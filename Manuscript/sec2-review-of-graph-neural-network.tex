...
\textbf{GGNN} (high vertex \& low edge computational complexity):
...
\textbf{GAT} (low vertex \& high edge computational complexity):
...
GAT moves the vertex update function $\gamma$ before the edge computation.
...
After the edge computation, GAT uses the aggreated vector $\boldsymbol{s}^l_i$ directly as the output hidden vector.

\textbf{GaAN} (high vertex \& edge computational complexity):
...
Moreover, GaAN aggregates three messages simutaneously, further increasing the edge computational complexity.
...
\begin{figure}
	\centering
	\subfloat[Neighbor sampling\label{fig:gnn_sampling_neigbhor_sampling}]{\includegraphics[height=4cm]{figs/illustration/layer_sampling.png}}
	%
	\subfloat[Graph sampling\label{fig:gnn_sampling_graph_sampling}]{\includegraphics[height=4cm]{figs/illustration/graph_sampling.png}}
